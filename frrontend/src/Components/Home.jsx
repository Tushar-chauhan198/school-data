...
export const Home = () => {
  ...
  return (
    <div>
      <Link to="/login">Login School Dashboard</Link>
      <br />
      <button onClick={handleSort}>Sort by Age</button> <br />
      <button onClick={() => handleGender("All")}>All</button>
      <button onClick={() => handleGender("Male")}>Sort by M</button>
      <br />
      <button onClick={() => handleGender("Female")}>Sort by F</button>
      ...
      <div className="Cards">
        {sortdata.map((e) => (
          <div key={e._id}>
            ...
            <p>Gender : {e.gender}</p>
            {e.class_id.map((el) => (
              <div key={el._id} style={{ border: "2px solid black" }}>
                <p>Grade : {el.Grade}</p>
